/* [nepasimprimer]
 * RCS Info
 * $Id: admsXml.c 1227 2011-05-11 16:05:44Z r29173 $
 *
 * Log
 * $Log$
...
static p_slist getlist_from_argv (const int argc,const char* *argv,const char* opt,char* argtype)
{
  ...
  for(i=1;i<argc;i++)
  {
    ...
    else if(!strcmp(opt,"-f") && value[0]!='-' && !argvprec_est_absorbant)
      myargvalue=adms_kclone(argv[i]);
    if(myargvalue)
    {
    ...
    if((strlen(value)==2)&&(!strncmp(value,"-f",2)||!strncmp(value,"-e",2)||!strncmp(value,"-I",2)))
      argvprec_est_absorbant=1;
    else
      argvprec_est_absorbant=0;
  }
...
struct sb* balisenouveau(const char **cp,int* L,const char *monfichier)
{
  ...
  balise.a0=NULL;
  balise.a=NULL;
  for(;*c&&!isspace((int)*c)&&strncmp(c,">",1)&&strncmp(c,"/>",2);c++);
  if(c-c0)
...
 /*[nepasimprimer]*/
  ...
  )
  {
    ...
    adms_message_usage(("  mailing-list=\"r29173@gmail.com\">\n"))
