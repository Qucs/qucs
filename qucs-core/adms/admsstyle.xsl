...
-->
<!--
  $Id: admsstyle.xsl 1227 2011-05-11 16:05:44Z r29173 $
-->
<!--
  $Log$
...
<html>
  ...
  <p>Symbols used:</p>
  <ul>
  <li><img src="adms-element.png"/>: adms element</li>
  <li><img src="adms-uid.png"/> property (constructor)</li>
  <li><img src="adms-scalar.png"/>: property (scalar)</li>
  <li><img src="adms-list.png"/> property (list)</li>
  </ul>
  <p>
...
<!--footer-->
  ...
  Please address any comments to <a href="mailto:r29173@gmail.com">r29173</a>.
  </p>
  </body>
</html>
</xsl:template>
  ...
  <span class="trigger">
    <xsl:choose>
      <xsl:when test="name(..)='uid'">
        <img> <xsl:attribute name="src">adms-uid.png</xsl:attribute></img>
        <xsl:variable name="id" select="concat(../../@name,@name)"/>
      </xsl:when>
      <xsl:otherwise>
        <img> <xsl:attribute name="src">adms-<xsl:value-of select="name(.)"/>.png</xsl:attribute> </img>
        <xsl:variable name="id" select="concat(../@name,@name)"/>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
