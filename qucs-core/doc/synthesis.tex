...
L = \dfrac{P_{in}}{P_{out}}
  = \dfrac{U_{in}^2}{Z_{in}}\cdot\dfrac{Z_{out}}{U_{out}^2}
  = \left( \dfrac{U_{in}}{U_{out}}\right)^2 \cdot\dfrac{Z_{out}}{Z_{in}}
\end{equation}
...
impedance, respectively. The pi-attenuator can be used for an
impedance ratio of:
\begin{equation}
...
\begin{align}
Z_2 & = \dfrac{2\cdot \sqrt{L\cdot Z_{in}\cdot Z_{out}}}{L^2 - 1} \\
Z_1 & = Z_{in}\cdot A - Z_2 \\
Z_3 & = Z_{out}\cdot A - Z_2 \\
    & \text{with} \qquad A = \dfrac{L + 1}{L - 1}
\end{align}
