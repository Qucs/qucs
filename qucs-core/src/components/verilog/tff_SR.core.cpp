/*
 * tff_SR.core.cpp - device implementations for tff_SR module
 *
...
/* Device constructor. */
tff_SR::tff_SR() : circuit (14)
{
  type = CIR_tff_SR;
}

/* Initialization of model. */
void tff_SR::initModel (void)
{
...
void tff_SR::restartDC (void)
{
}

/* Initialize Verilog-AMS code. */
void tff_SR::initVerilog (void)
{
...
void tff_SR::loadVariables (void)
{
...
void tff_SR::initializeModel (void)
{
#if defined(_DYNAMIC)
#endif
{
...
void tff_SR::initializeInstance (void)
{
}

/* Evaluate Verilog-AMS equations in initial step. */
void tff_SR::initialStep (void)
{
}

/* Evaluate Verilog-AMS equations in final step. */
void tff_SR::finalStep (void)
{
}

/* Evaluate Verilog-AMS equations in analog block. */
void tff_SR::calcVerilog (void)
{

/* ----------------- evaluate verilog analog equations -------------------- */
{
double m00_tanh(d00_tanh0,(TR_H*(((NP(T)*NP(QB))+((1-NP(T))*NP(QO)))-0.5)))
#if defined(_DERIVATE)
double m10_tanh(d10_tanh0,d00_tanh0,(TR_H*(((NP(T)*NP(QB))+((1-NP(T))*NP(QO)))-0.5)))
#endif
_load_static_residual1(Dsig,((-0.5)*(1+d00_tanh0)));
#if defined(_DERIVATE)
_load_static_jacobian1(Dsig,QO,((-0.5)*(TR_H*((1-NP(T))))*d10_tanh0));
_load_static_jacobian1(Dsig,QB,((-0.5)*(TR_H*(NP(T)))*d10_tanh0));
_load_static_jacobian1(Dsig,T,((-0.5)*(TR_H*((NP(QB))+(-1.0)*NP(QO)))*d10_tanh0));
#endif
}
...
void tff_SR::saveOperatingPoints (void)
{
  // save global instance operating points
}

/* Load operating points. */
void tff_SR::loadOperatingPoints (void)
{
}

/* Calculate operating points. */
void tff_SR::calcOperatingPoints (void)
{
}

/* Initialization of AC analysis. */
void tff_SR::initAC (void)
{
  allocMatrixMNA ();
}

/* Perform AC calculations. */
void tff_SR::calcAC (nr_double_t frequency)
{
  setMatrixY (calcMatrixY (frequency));
}

/* Compute Y-matrix for AC analysis. */
matrix tff_SR::calcMatrixY (nr_double_t frequency)
{
  _freq = frequency;
  saveOperatingPoints ();
  matrix y (14);
...
void tff_SR::initSP (void)
{
  allocMatrixS ();
}

/* Perform S-parameter calculations. */
void tff_SR::calcSP (nr_double_t frequency)
{
  setMatrixS (ytos (calcMatrixY (frequency)));   
}

/* Initialization of transient analysis. */
void tff_SR::initTR (void)
{
  setStates (2 * 14 * 14);
  initDC ();
}
...
void tff_SR::calcTR (nr_double_t)
{
  doHB = 0;
  doAC = 1;
  doTR = 1;
...
matrix tff_SR::calcMatrixCy (nr_double_t frequency) 
{
  _freq = frequency;
  matrix cy (14);
