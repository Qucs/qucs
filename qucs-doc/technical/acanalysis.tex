...
\begin{equation}
U_L = j\omega L\cdot I_L + j\omega\cdot \sum_{n=1}^N k_n\cdot\sqrt{L\cdot L_n}\cdot I_{L,n}
\end{equation}
where $U_L$ and $I_L$ is the voltage across and the current through
...
Realizing this approach with the MNA matrix is straight forward:
Every inductance $L$ needs an additional matrix row. The corresponding
element in the $D$ matrix is $j\omega L$. If two inductors are coupled
the cross element in the $D$ matrix is
$j\omega\cdot k\cdot\sqrt{L_1\cdot L_2}$. For two coupled inductors
this yields:
...
. & . & . & . &  0 & +1\\
. & . & . & . &  0 & -1\\
+1 & -1 & 0 & 0 & j\omega L_1 & j\omega\cdot k\cdot\sqrt{L_1\cdot L_2} \\
0 & 0 & +1 & -1 & j\omega\cdot k\cdot\sqrt{L_1\cdot L_2} & j\omega L_2 \\
\end{bmatrix}
\cdot
\begin{bmatrix}
V_1\\
...
Obviously, this approach has an advantage: It also works for
zero inductances and for unity coupling factors. It has the
disadvantage that it enlarges the MNA matrix.
