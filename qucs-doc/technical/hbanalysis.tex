...
<33>\quad \qquad\qquad\qquad tempr = wr*data[j-1] - wi*data[j]; \\
...
\begin{tabular}{ll}
 ...
 Y = calcTransMatrix();     & // transimpedance matrix of linear circuit \\
 ...
 & \\
 loop: & \\
\quad I = FFT(i);                       & // current into frequency domain \\
\quad Q = FFT(q);                       & // charge into frequency domain \\
\quad E = Is + Y*V + j*Omega*Q + I;     & // HB equation \\
\quad if (abs(E) < Eterm)  break;       & // convergence reached ? \\
\quad JG = mFFT(GJacobian(v, i));       & // create Jacobians and transform... \\
\quad JQ = mFFT(QJacobian(v, i));       & // ... them into frequency domain \\
\quad J = Y + j*Omega*JQ + JG;          & // calculate overall Jacobian \\
\quad V = V - invert(J) * Error;        & // Newton Raphson iteration step \\
\quad v = IFFT(V);                      & // voltage into time domain \\
\quad i = nonlinearCurrent(v);          & // use component models to get... \\
\quad q = nonlinearCharge(v);           & // ... values for next iteration \\
\quad goto loop; & \\
 & \\
Va = invert(Y) * I                      & // AC simulation to get all voltages \\
\end{tabular}
}
