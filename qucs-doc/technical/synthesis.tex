...
\subsection{Lumped equivalent circuits of the $\lambda/4$ transmission line}

\noindent The lumped equivalent circuit may be found by comparing the $\lambda/4$ transmission line to the $\pi$ and T-type networks in terms of their ABCD parameters.

\begin{figure}[H]
\centering
\includegraphics[width=80mm]{Pi-equivalent-ckt}
\caption{$\pi$-type equivalent circuit of a $\lambda/4$ transmission line}
\label{fig:pi-equivalent-ckt}
\end{figure}


\noindent The ABCD matrix of a transmission line is given by:
...
\noindent For $L = \lambda/4$, it reduces to:

\begin{equation}
A_{\lambda/4} = \begin{pmatrix}
    0       & j \cdot Z_0 \\
    \frac{j}{Z_0} & 0
\end{pmatrix}
\label{eq:ABCD_QW}
...
\begin{pmatrix}
     Y_2 \cdot Z_1 + 1       & Z_1 \\
    (Y_1 \cdot Z_1 + 1)\cdot Y_2 + Y_1  & Y_1 \cdot Z_1 + 1
\end{pmatrix}
\label{eq:ABCD_PI}
\end{equation}

\noindent By comparing \ref{eq:ABCD_QW} and \ref{eq:ABCD_PI} we get the following system of equations:

\begin{equation}
A_{\lambda/4} = A_{\pi} \Longrightarrow
\begin{cases} Y_2 \cdot Z_1 + 1 = 0 \\ Z_1 =  j \cdot Z_0 \\ (Y_1 \cdot Z_1 + 1)\cdot Y_2 + Y_1 = \frac{j}{Z_0} \\ Y_1 \cdot Z_1 + 1 = 0\end{cases} 
\label{eq:qw-pi-equivalent-equations}
\end{equation}

\noindent Since $Z_1 =  j \cdot Z_0$, the series element must be an inductor such that $L_{\pi} = \frac{Z_0}{\omega_0}$. From the first fourth equations in \ref{eq:qw-pi-equivalent-equations}, the shunt elements must be capacitors of the same value.

\begin{equation}
Y_2 \cdot Z_1 + 1 = 0 \Longrightarrow Y_2 = -\frac{1}{Z_1} = j \cdot \frac{1}{Z_0} \Longrightarrow C_{\pi} = \frac{1}{\omega_0 \cdot Z_0}
\end{equation}

\noindent In order to check the equivalent circuit is valid, it is needed to verify the above solutions using the third equation in \ref{eq:qw-pi-equivalent-equations}:

\begin{equation}
(Y_1 \cdot Z_1 + 1) \cdot Y_2  + Y_1 = \frac{j}{Z_0} \Longleftrightarrow \left( \frac{j}{Z_0} \cdot j \cdot Z_0 + 1\right) \cdot \frac{j}{Z_0} + \frac{j}{Z_0} = \frac{j}{Z_0} \Longleftrightarrow  \frac{j}{Z_0} = \frac{j}{Z_0}
\end{equation}

\noindent Now, it is possible to find the T-type equivalent using \ref{tbl:pi-to-tee-transformation}:

\begin{align}
           Z_1^T &= \frac{Z_A^{\pi} \cdot Z_B^{\pi}}{Z_A^{\pi} + Z_B^{\pi} + Z_C^{\pi}} = j \cdot Z_0 \Longrightarrow L_{T} = \frac{Z_0}{\omega_0}\\
           Z_2^T &= \frac{Z_B^{\pi} \cdot Z_C^{\pi}}{Z_A^{\pi} + Z_B^{\pi} + Z_C^{\pi}} = j \cdot Z_0 = Z_1\\
           Z_3^T &= \frac{Z_A^{\pi} \cdot Z_C^{\pi}}{Z_A^{\pi} + Z_B^{\pi} + Z_C^{\pi}} = -j \cdot Z_0 \Longrightarrow C_{T} = \frac{Z_0}{\omega_0}
         \end{align}

\noindent where

\begin{align}
   Z_A^{\pi} &= \frac{1}{Y_1} = 1/(j \cdot \omega_0 \cdot C_{\pi}) \\
   Z_B^{\pi} &= Z_1 = j \cdot \omega_0 \cdot L_{\pi} \\
   Z_C^{\pi} &= \frac{1}{Y_2} = 1/(j \cdot \omega_0 \cdot C_{\pi})
\end{align}

\begin{figure}[H]
\centering
\includegraphics[width=80mm]{TL_Pi_Tee_conversion}
\caption{Conversion between $\pi$-type and T-type circuit equivalents of the $\lambda\/4$ transmission line}
\label{fig:pi-t-equivalent-ckt}
\end{figure}



\subsection{Lumped equivalent circuit of the $\lambda/8$ transmission line}

\noindent As with the $\lambda/4$ transmission line, the equivalent circuit of the $\lambda/8$ transmission line is calculated using the ABCD matrix. From \ref{eq:ABCD_TL}

\begin{equation}
A_{\lambda/8} = \frac{1}{2} \cdot \begin{pmatrix}
    \sqrt{2}       & j \cdot Z_0 \cdot \sqrt{2}\\
    \frac{j \sqrt{2}}{Z_0} & \sqrt{2} 
\end{pmatrix}
\label{eq:ABCD_L8}
\end{equation}

\begin{equation}
A_{\lambda/8} = A_{\pi} \Longrightarrow
\begin{cases} Y_2 \cdot Z_1 + 1 = \frac{\sqrt{2}}{2} \\ Z_1 =  j \cdot Z_0 \cdot \frac{\sqrt{2}}{2} \\ (Y_1 \cdot Z_1 + 1)\cdot Y_2 + Y_1 = \frac{j \cdot \sqrt{2}}{2 \cdot Z_0} \\ Y_1 \cdot Z_1 + 1 = \frac{\sqrt{2}}{2}\end{cases} 
\label{eq:l8-pi-equivalent-equations}
\end{equation}

\noindent By comparing \ref{eq:ABCD_L8} and \ref{eq:ABCD_PI} we get the following system of equations:

\noindent Since $ Z_1 =  j \cdot Z_0 \cdot \frac{\sqrt{2}}{2}$, the series element must be an inductor such that: $L_{\pi} = \frac{Z_0 \cdot \sqrt{2}}{2 \cdot \omega_0}$

\begin{equation}
 Y_2 \cdot Z_1 + 1 = \frac{\sqrt{2}}{2} \Longrightarrow Y_2 =  \frac{\frac{\sqrt{2}}{2} - 1}{Z_1} = j \cdot \frac{\sqrt{2}-1}{Z_0} \Leftrightarrow C_{\pi} = \frac{\sqrt{2} - 1}{Z_0 \cdot \omega_0}
\end{equation}



\noindent Now, it is possible to find the T-type equivalent using the transformation equations \ref{tbl:pi-to-tee-transformation}:

\begin{align}
   Z_1^T &= \frac{1}{Y_1} = \frac{Z_A^{\pi} \cdot Z_B^{\pi}}{Z_A^{\pi} + Z_B^{\pi} + Z_C^{\pi}} = j \cdot \frac{Z_0}{\sqrt{2} + 1} \Longrightarrow L_T = \frac{Z_0 \cdot (1 - \sqrt{2})}{\omega_0}\\
   Z_2^T &= Z_1 = \frac{Z_B^{\pi} \cdot Z_C^{\pi}}{Z_A^{\pi} + Z_B^{\pi} + Z_C^{\pi}} = j \cdot \frac{Z_0}{\sqrt{2} + 1} = Z_1\\
   Z_3^T &= \frac{1}{Y_2} =\frac{Z_A^{\pi} \cdot Z_C^{\pi}}{Z_A^{\pi} + Z_B^{\pi} + Z_C^{\pi}} = -j \cdot \frac{Z_0 \cdot \sqrt{2}}{(\sqrt{2}+1)(\sqrt{2}-1)}\Longrightarrow C_T = \frac{\sqrt{2}}{2 \cdot \omega_0 \cdot Z_0}
\end{align}
