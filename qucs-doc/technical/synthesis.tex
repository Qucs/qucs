...
The equations for a Chebyshev type I filter are defined recursively.
...
Quarter wavelength filters are based on the fact that short and open $\lambda/4$ stubs are equivalent to series and parallel resonant tanks, respectively. Once given the normalized $g_i$ coefficients of the filter prototype, it is quite straightforward to obtain the impedance of the short/open stubs, depending on the desired filter mask (bandpass/notch). Figure \ref{eq:QWfilter} illustrates the topology of a quarter wavelength filter.\\
...
\caption{Impedance translation produced by a series/shunt capacitor and inductor}
...
\caption{Impedance translation caused by a transmission line and an open/short circuited stub}
\label{fig:SmithTL}
\end{figure}
...
\noindent Finally, if needed, the stubs may be split into balanced stubs as follows:
...
\noindent where $\theta$ is the electrical length of a single transmission line (typically $\lambda/4$). The ${\Gamma_n}$ weights define the shape of $\Gamma(\theta)$ and they are tailored so as to get a binomial or a Chebyshev overall response.
...
\noindent The overall reflection coefficient is set to:
