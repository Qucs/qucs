...
area.  Above there are the menu bar as well as some icons.
...
To take effect of the language and font seetings the application must
...
different types of transmission lines (e.g. micrstrips, coaxial
cables).
...
netlists and schematics from and to other CAD/EDA software.
...
In the given circuit the ground symbol is placed at the negative
terminal of the DC voltage source.
...
\tutsubsubsection{Labeling wires}
...
menu entry.  Aftwards double click the \textbf{R1} resistor.  This
...
engineering (1k) number notation can be choosen.  Some units are also
...
Additionally the x-axis has been chosen to be logarithmical.  The
...
maximum value.  If you want to know the appropiate values it is
...
configure the percision as well as the number notation of the
displayed values.
...
\begin{figure}[ht]
  ...
  \caption{Output characteristics of a NPN bipolar transitor}
...
the bipolar transistor used in the previous DC sections.
...
For maximum AC performance we use as a rule of thumb the bias point
with the maximum DC current gain.  For this in fig.~\ref{fig:igain_ib}
the DC current gain from fig.~\ref{fig:igain_4} is plotted versus the
base current \textbf{Ib} choosing \textbf{Beta} in the diagram dialog
instead of \textbf{Beta\_vs\_Ic}.  The appropiate base current shown
in the marker is $140\micro\ampere$.
...
$40\micro\ampere$, $140\micro\ampere$ and $240\micro\ampere$.
...
wheras port 1 is the base and port 2 the collector.  The side
...
Performing the actual AC analysis is as easy as dragging and droping an
...
simualtion setups cover perfectly each other.  Again the transit
