...
qf_poly &
  qf_poly::operator = (const qf_poly & P) 
{
...
qf_poly 
  operator * (qf_poly P, const   qf_double_t & m)
{
  if (P. rep == NONE)
  {
...
qf_poly &
  qf_poly::operator *= (const   qf_double_t & m)
{
  if (rep == NONE)
  {
    std::cout << "qf_poly::* (scalar) used on a NONE polynom.\n" ;
  ...
  qf_poly::operator << (unsigned n)
{
  ...
  if ((rep == ROOTS) || (rep == BOTH))
  {
    ...
    if (nbz != 0)     // We did not found a sufficient number of zeros
    {
      std::cout << "Warning: << by " << n << " asked for but only " \
		<< n - nbz << " possible.\n" ;
      R. d += nbz ;
  ...
  qf_poly::sqr ()
{
  ...
  if ((* this) != P)	    // Contains odd order terms
  {
...
void
  qf_poly::div (  qf_double_t r,  qf_double_t i)
...
# endif
    ...
    if ((fabs (rts [k] - r) > ROOT_TOL) || 
	(fabs (rts [k + 1] - i) > ROOT_TOL))
    {
      rtsp [j] = rts [k] ;
      rtsp [j + 1] = rts [k + 1] ;
      j += 2 ;
  ...
  smpf (qf_poly & N, qf_poly & D) 
{
  ...
  for (unsigned i = 0 ; i < 2 * dN ; i += 2)
  {
    ...
    {
      printf ("N. rts [%d] = %.9f, ", i, N. rts [i]) ;
      printf ("D. rts [%d] = %.9f\n", j, D. rts [j]) ;
      printf ("N. rts [%d] = %.9f, ", i + 1, N. rts [i + 1]) ;
      printf ("D. rts [%d] = %.9f\n", j + 1, D. rts [j + 1]) ;
...
/* This function calculates P(X) = Σ a[i] X^i (sum form) out of the roots
  (product form) P(X) = k Π (X - r[i])
*/

void
  qf_poly::to_coeff (void)
{
  if (rep == NONE)
  {
...
void qf_poly::s_scm (qf_matrix& m) {
  unsigned int i, j;
  for (i = 0 ; i < m.n; i++)
    for (j = 0 ; j < m.n; j++)
      m (i, j) = 0;
...
void qf_poly::qf_bcm (qf_matrix& m) {
  ...
  while (not_converged) 
  {
	...
	row_norm = fabs (m (i, i - 1)) ;
      ...
      {
	row_norm = fabs (m (i, i - 1)) + fabs (m (i, d - 1));
      }
