...
public:
  ...
  bool insideDiagramPh(Graph::iterator const& , float*, float*) const; //aqui
  bool newcoordinate(Graph::iterator const& , float*, float*) const; //aqui
  void phasorscale(); //aqui
  void findaxisA(Graph*); //aqui
  bool findmatch(Graph* , int);
  //bool findnewfreq(Graph*);
  void findfreq(Graph*);
  ...
  QString sfreq;//aqui
  double *freq=nullptr;//aqui
  int nfreqt,nfreqa;
  int x3, y3, sc;
  Axis  xAxis, yAxis, zAxis;   // axes (x, y left, y right)
  Axis  xAxisV, yAxisV, zAxisV, xAxisI, yAxisI, zAxisI, xAxisP, yAxisP, zAxisP, xAxisZ, yAxisZ, zAxisZ, *xAxisA, *yAxisA, *zAxisA;//for Phasor diagram
