                    ...
                    : QDialog(parent, 0, TRUE, Qt::WDestructiveClose)
{
  ...
  if(Diag->Name == "Tab") {
    ...
    if((Diag->Name=="Rect") || (Diag->Name=="PS") || (Diag->Name=="SP") || (Diag->Name=="Curve") || (Diag->Name=="Phasor") ) {
  ...
  if(Diag->Name == "Phasor" || Diag->Name == "Waveac") //for Phasor and waveac will have a new input for frequency
  {
  ...
  if(Diag->Name == "Phasor")
  {
    ...
    if(testvar(".VA")) inputP->setChecked(true);
...
void DiagramDialog::slotReadVars(int)
{
  ...
  do {
    ...
    //waveac only show voltage(".v") and current(".i")   
    if(Var.length()>0)
    {
...
{
  if(state == 2) {//if check add graph of type ".VA" if exist
    addvar(".VA");
  }
  else {//if uncheck remove graph of type ".VA"
    remvar(".VA");
  }
}
