/*!
  \class PhasorDiagram
  \brief The PhasorDiagram class implements the Phasor diagram
*/
/***************************************************************************
                                phasordiagram.cpp
                               -----------
...
// ------------------------------------------------------------
/*void PhasorDiagram::calcCoordinate(const double* , const double* yD, const double*,
				  float *px, float *py, Axis const *pa) const//aqui
{
 double yr = yD[0];
 double yi = yD[1];
 	

  *px = float((yr - xAxisA->low) / (xAxisA->up - xAxisA->low) * double(x2));
...
void PhasorDiagram::calcData(Graph *g)
{
  ...
  double yf[] = {0.0,0.0};//points of the begining of the vector 
  float xn,yn;
  float f1,f2; 
  char a=0; 
  double *yp;

  z = QString::compare(g->axis(0)->Var,"acfrequency",Qt::CaseInsensitive); //meaning that only work in AC 
  if(z != 0 )
  {
    sfreq = "0 Hz";
    p->setGraphEnd();
  ...
  for(z=0;z<nfreqt;z++)
  {
	...
	return;
      ...
      p->setScr(f1, f2);
      if(a==1)//if the point(0,0) of vector isn't inside diagram will try to find new coordinates closest to the old coordinates that is within diagram
      {
        if(newcoordinate(p,&xn,&yn)) (p-1)->setScr(xn, yn);//if find a new point saves
	else //will not create a vector
	{
          --p;
	  continue;  
	}
      ...
      else//will find a new one closest to the old ones
      {
        if(newcoordinate(p,&xn,&yn))//if find a new point saves
        {
	  (p++)->setScr(xn, yn);
          (p++)->setScrX(-1);
        }
        else --p;//will not create a vector
      }
      (p++)->setBranchEnd();
    }
  }
