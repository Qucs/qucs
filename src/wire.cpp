...
private: // Symbol
	...
	pos_t portPosition(unsigned i) const{ untested();
		switch(i){
...
Wire::~Wire()
{ untested();
	if(port(0).isConnected()){ untested();
		unreachable();
		incomplete();
	}else if(port(1).isConnected()){ untested();
		unreachable();
		incomplete();
	}else{ untested();
	}
...
{ untested();
	...
	if(in_order(n0, other, n1)){ untested();
		return clone();
	}else if(in_order(other, n1, n0) && _port1->numPorts()==2){ untested();
